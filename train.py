...
# zjb 注释原因：使用IntTensor在面对长整型字符串的时候会导致不能在GPU上运算的问题，具体表现为：
...
if params.cuda and torch.cuda.is_available():
    ...
    image = image.cuda()
    text = text.cuda()  # 确保数据运行GPU上 zjb
    length = length.cuda()  # 确保数据运行GPU上 zjb
...
def val(net, dataset, criterion, max_iter=100):
    ...
    net.eval()
    data_loader = torch.utils.data.DataLoader(dataset, shuffle=True, batch_size=params.batchSize,
...
if __name__ == "__main__":
    for epoch in range(params.nepoch):
        ...
        if (epoch + 1) % params.saveInterval == 0:
            torch.save(crnn.state_dict(), '{0}/netCRNN_{1}_{2}_{3}.pth'.format(params.expr_dir, epoch + 1, i, acc))

# python train.py --trainroot train_all_lmdb --valroot val_all_lmdb
# nohup python train.py --trainroot train_all_lmdb --valroot val_all_lmdb >train.log 2>&1 &

# python train.py --trainroot train_mobiles_lmdb --valroot val_mobiles_lmdb
# nohup python train.py --trainroot train_mobiles_lmdb --valroot val_mobiles_lmdb >train.log 2>&1 &
